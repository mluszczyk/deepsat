...
def _expand_ids(ids):
    ...
    for id in ids:
        if '-' in id:
            # we split two numbers - this is a computer range 
            comp_list = id.split('-')
            begin, end = [int(token) for token in comp_list]
            result.extend(_pad_zeros(range(begin, end+1), len(comp_list[1])))
        else:
            result.append(id)
    print("Number of nodes {}, nodes {}".format(len(result),result))
    return result
...
def _expand_nodelist(nodelist):
    ...
    print("Number of nodes {}, nodes after expansion {}".format(len(result),result))
    return result
