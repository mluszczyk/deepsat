function train {
	...
	MODEL_NAME="1809_relu_softmax_henryk/${DIFFICULTY}_l${LEVEL_NUMBER}_sa${ATTENTION}_ra${RELU_ATTENTION}_ss${SOFTMAX_SEBASTIAN}_sc${SOFTMAX_CHRISTIAN}_t${TEMPERATURE//./}_t${TPU_NAME}"
...
function sr50 {
	TEMPLATE='python neurosat_tpu.py --use_tpu=True --tpu=$TPU_NAME --train_file=gs://$BUCKET_NAME/sr_50_direct_from_prom/sr_50/train_1_sr_50.tfrecord.gz --test_file=gs://$BUCKET_NAME/sr_50_direct_from_prom/sr_50/train_2_sr_50.tfrecord.gz --train_steps=1200000 --test_steps=1000 --model_dir=gs://$BUCKET_NAME/$MODEL_NAME --export_dir=gs://$BUCKET_NAME/export/$MODEL_NAME --variable_number=50 --clause_number=500 --train_files_gzipped=True --test_files_gzipped=True --batch_size=64 --export_model --attention=$ATTENTION --relu_attention=$RELU_ATTENTION --softmax_sebastian=$SOFTMAX_SEBASTIAN --softmax_christian=$SOFTMAX_CHRISTIAN --temperature=$TEMPERATURE --level_number=$LEVEL_NUMBER'
	train 50 "$TEMPLATE" "$1" "$2" "$3" "$4" "$5" "$6" "$7"
}

function sr70 {
	TEMPLATE='python neurosat_tpu.py --use_tpu=True --tpu=$TPU_NAME --train_file=gs://$BUCKET_NAME/sr_70_gz/train/train_1_sr_70.tfrecord.gz --test_file=gs://$BUCKET_NAME/sr_70_gz/test/test_9_sr_70.tfrecord.gz --train_steps=600000 --test_steps=1000 --model_dir=gs://$BUCKET_NAME/$MODEL_NAME --export_dir=gs://$BUCKET_NAME/export/$MODEL_NAME --variable_number=70 --clause_number=700 --train_files_gzipped=True --test_files_gzipped=True --batch_size=64 --attention=$ATTENTION --relu_attention=$RELU_ATTENTION --softmax_sebastian=$SOFTMAX_SEBASTIAN --softmax_christian=$SOFTMAX_CHRISTIAN  --temperature=$TEMPERATURE --export_model --level_number=$LEVEL_NUMBER'
	train 70 "$TEMPLATE" "$1" "$2" "$3" "$4" "$5" "$6" "$7"
}

function sr100 {
	TEMPLATE='python neurosat_tpu.py --use_tpu=True --tpu=$TPU_NAME --train_file=gs://$BUCKET_NAME/sr_100_gz/train/train_1_sr_100.tfrecord.gz --test_file=gs://$BUCKET_NAME/sr_100_gz/test/test_9_sr_100.tfrecord.gz --train_steps=1200000 --test_steps=1000 --model_dir=gs://$BUCKET_NAME/$MODEL_NAME --export_dir=gs://$BUCKET_NAME/export/$MODEL_NAME --variable_number=100 --clause_number=1000 --train_files_gzipped=True --test_files_gzipped=True --batch_size=32 --attention=$ATTENTION --relu_attention=$RELU_ATTENTION --softmax_sebastian=$SOFTMAX_SEBASTIAN --softmax_christian=$SOFTMAX_CHRISTIAN --temperature=$TEMPERATURE  --level_number=$LEVEL_NUMBER --export_model'
