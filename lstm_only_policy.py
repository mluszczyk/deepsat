...
class Graph:
    def __init__(self):
        ...
        self.policy_loss = tf.losses.sigmoid_cross_entropy(
            self.policy_labels, self.policy_logits)
        ...
        self.policy_top1_error = 1.0 - tf.reduce_sum(tf.gather_nd(
            self.policy_labels,
            tf.stack([tf.range(BATCH_SIZE), tf.argmax(self.policy_probabilities, axis=1, output_type=tf.int32)],
                     axis=1))) / BATCH_SIZE

        self.policy_error = tf.reduce_sum(tf.abs(
            tf.round(self.policy_probabilities) - self.policy_labels)) / (
                BATCH_SIZE * (VARIABLE_NUM * 2))
...
def gen_cnfs_with_labels():
    cnfs, solutions = get_random_sat_kcnfs(
        BATCH_SIZE, CLAUSE_SIZE, VARIABLE_NUM, CLAUSE_NUM,
    ...
    for solution in solutions:
        ...
        for v in range(1, VARIABLE_NUM + 1):
            for sv in [v, -v]:
                result = 1.0 if sv in solution else 0.0
                label.append(result)
        policy_labels.append(label)
    return cnfs, policy_labels
...
def main():
    ...
    with tf.Session() as sess:
        ...
        @timed
        def nn_train(cnfs, policy_labels):
        ...
        @timed
        def complete_step():
            cnfs, policy_labels = gen_cnfs_with_labels()
            nn_train(cnfs, policy_labels)
